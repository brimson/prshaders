...
)
{
	...
	float4 wPos = mul(float4(inPos.xyz * PosUnpack, 1), World);
...
VS_OUTPUT basicVertexShaderOvergrowth
(
	float4 inPos : POSITION0,
	float3 normal : NORMAL,
	float2 tex0 : TEXCOORD0
)
{
	...
	float FogValue = Out.Pos.z; // length(WorldSpaceCamPos.xyz - wPos.xyz);
	Out.Fog = calcFog(FogValue);
	Out.Tex0 = tex0;
...
technique defaultTechnique
{
	...
	{
		#ifdef OVERGROWTH
			vertexShader = compile vs_2_a basicVertexShaderOvergrowth();
		#else
