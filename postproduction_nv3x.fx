...
CREATE_SAMPLER(PostProductionNV3X_Sampler_0_Bilinear, Texture_0, CLAMP, LINEAR)

CREATE_SAMPLER(PostProductionNV3X_Sampler_1_Bilinear, Texture_1, CLAMP, LINEAR)
CREATE_SAMPLER(PostProductionNV3X_Sampler_1_Bilinear_Wrap, Texture_1, WRAP, LINEAR)

CREATE_SAMPLER(PostProductionNV3X_Sampler_2_Bilinear, Texture_2, CLAMP, LINEAR)
CREATE_SAMPLER(PostProductionNV3X_Sampler_2_Bilinear_Wrap, Texture_2, WRAP, LINEAR)

CREATE_SAMPLER(PostProductionNV3X_Sampler_3_Bilinear, Texture_3, CLAMP, LINEAR)
...
float4 Tinnitus_PS(VS2PS_Quad Input) : COLOR
{
	...
	for(int i = 0; i < 8; i++)
	{
		Blur += FilterKernel[i].w * tex2D(PostProductionNV3X_Sampler_0_Bilinear, Input.TexCoord0.xy + 0.02 * FilterKernel[i].xy);
	}

	float4 Color = tex2D(PostProductionNV3X_Sampler_0_Bilinear, Input.TexCoord0);
...
{
	return tex2D(PostProductionNV3X_Sampler_0_Bilinear, Input.TexCoord0);
}

float4 Glow_Material_PS(VS2PS_Quad Input) : COLOR
{
	float4 Diffuse = tex2D(PostProductionNV3X_Sampler_0_Bilinear, Input.TexCoord0);
...
/*
	...
	{
		float3 WorldPosition = tex2D(PostProductionNV3X_Sampler_0_Bilinear, Input.TexCoord0).xyz;
		...
		return tex2D(PostProductionNV3X_Sampler_1_Bilinear, float2(Coord, 0.0)) * _FogColor.rgbb;
	}
...
PS2FB_Combine Thermal_Vision_PS(VS2PS_Quad_3 Input)
{
	...
	float4 Image = tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord);

	if (_Interference <= 1)
	{
		float2 Pos = Input.TexCoord0;
		float Random = tex2D(PostProductionNV3X_Sampler_2_Bilinear_Wrap, Pos) - 0.2;
		if (_Interference < 0) // thermal imaging
		{
			float HOffset = 0.001;
			float VOffset = 0.0015;
			Image *= 0.25;
			Image += tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord + float2( HOffset, VOffset)) * 0.0625;
			Image += tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord - float2( HOffset, VOffset)) * 0.0625;
			Image += tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord + float2(-HOffset, VOffset)) * 0.0625;
			Image += tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord + float2( HOffset, -VOffset)) * 0.0625;
			Image += tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord + float2( HOffset, 0.0)) * 0.125;
			Image += tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord - float2( HOffset, 0.0)) * 0.125;
			Image += tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord + float2( 0.0, VOffset)) * 0.125;
			Image += tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord - float2( 0.0, VOffset)) * 0.125;
		...
		{
			float Noise = tex2D(PostProductionNV3X_Sampler_1_Bilinear_Wrap, Input.TexCoord1) - 0.5;
...
PS2FB_Combine Thermal_Vision_Gradient_PS(VS2PS_Quad_3 Input)
{
	...
	if ( _Interference >= 0 && _Interference <= 1 )
	{
		...
		Distort /= 1.0 + _DistortionScale * abs(Pos.y);
		ImgCoord.x += _DistortionScale * Noise * Distort;
		float4 Image = dot(float3(0.3, 0.59, 0.11), tex2D(PostProductionNV3X_Sampler_0_Bilinear, ImgCoord).rgb);
		float4 Intensity = (_Interference * Random + Image * (1.0 - _TVAmbient) + _TVAmbient);
		float4 GradientColor = tex2D(PostProductionNV3X_Sampler_3_Bilinear, float2(Intensity.r, 0.0f));
		Output.Col0 = float4( GradientColor.rgb, Intensity.a );
	}
	else
	{
		Output.Col0 = tex2D(PostProductionNV3X_Sampler_0_Bilinear, Input.TexCoord2);
	}
...
{
	float4 Sample0 = tex2D(PostProductionNV3X_Sampler_0_Bilinear, Input.TexCoord0);
	float4 Sample1 = tex2D(PostProductionNV3X_Sampler_1_Bilinear, Input.TexCoord0);
	float4 Sample2 = tex2D(PostProductionNV3X_Sampler_2_Bilinear, Input.TexCoord0);
	float4 Sample3 = tex2D(PostProductionNV3X_Sampler_3_Bilinear, Input.TexCoord0);
