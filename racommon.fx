...
float4 calcShadowProjection(float4 pos, uniform float BIAS = -0.001, uniform bool ISOCCLUDER = false)
{
...
//tl: Make _sure_ pos and matrices are in same space!
float4 calcShadowProjectionExact(float4 pos, uniform float BIAS = -0.001)
{
	float4 texShadow1 =  mul(pos, ShadowTrapMat);
