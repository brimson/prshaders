...
float4 psDx9_tr_opticsNoBlurCircle(VS2PS_tr_blit indata) : COLOR
{
   ...
   float1 dist = length((indata.TexCoord0-0.5)*float2(aspectRatio,1)); // get distance from the center of the screen
   float1 blurAmount = saturate((dist - radius1)/(radius2-radius1))*blurAmountMod;
