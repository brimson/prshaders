...
void skinSoldierForPPtangent(uniform int NumBones, in APP2VStangent indata, in float3 lightVec, out float3 Pos, out float3 Normal, out float3 SkinnedLVec, out float4 wPos, out float3 HalfVec)
{
	...
	float3 tanEyeVec = mul(worldEyePos.xyz - wPos.xyz, mat);
...
VS2PS_PP VShader_HemiAndSunPP(APP2VS indata, uniform int NumBones)
{
	...
	skinSoldierForPP(NumBones, indata, -sunLightDir.xyz, Pos, Normal, SkinnedLVec);
	...
	outdata.HalfVec = normalize(normalize(objectEyePos.xyz-Pos) + SkinnedLVec);
...
float4 PShader_HemiAndSunAndShadowPP(VS2PS_PP_Shadow indata) : COLOR
{
	...
	staticSamples.x = tex2D(sampler2, indata.ShadowTex.xy + float2(-texel.x*1, -texel.y*2)).b;
	staticSamples.y = tex2D(sampler2, indata.ShadowTex.xy + float2( texel.x*1, -texel.y*2)).b;
	staticSamples.z = tex2D(sampler2, indata.ShadowTex.xy + float2(-texel.x*1,  texel.y*2)).b;
	staticSamples.w = tex2D(sampler2, indata.ShadowTex.xy + float2( texel.x*1,  texel.y*2)).b;
...
float4 PShader_HemiAndSunAndColorPP(VS2PS_PP indata) : COLOR
{
	...
	float4 totalcolor = saturate(float4(suncol*groundcolor.a*groundcolor.a+ambientColor.rgb*hemicolor.rgb, specular));	// Do something with spec-alpha later on
...
float4 PShader_HemiAndSunAndShadowAndColorPP(VS2PS_PP_Shadow indata) : COLOR
{
	...
	float4 staticSamples;
	staticSamples.x = tex2D(sampler3, indata.ShadowTex.xy + float2(-texel.x*1, -texel.y*2)).b;
	staticSamples.y = tex2D(sampler3, indata.ShadowTex.xy + float2( texel.x*1, -texel.y*2)).b;
	staticSamples.z = tex2D(sampler3, indata.ShadowTex.xy + float2(-texel.x*1,  texel.y*2)).b;
	staticSamples.w = tex2D(sampler3, indata.ShadowTex.xy + float2( texel.x*1,  texel.y*2)).b;
	staticSamples.x = dot(staticSamples.xyzw, 0.25);
...
//return avgShadowValue;
	float4 color = tex2D(sampler2, indata.Tex0);
	float4 totalcolor = saturate(float4(suncol*totShadow+ambientColor.rgb*hemicolor.rgb, specular));	// Do something with spec-alpha later on
	totalcolor.rgb *= color.rgb;
...
VertexShader vsArray_HemiAndSunPP[2] = { compile vs_2_a VShader_HemiAndSunPP(1), compile vs_2_a VShader_HemiAndSunPP(2) };
VertexShader vsArray_HemiAndSunAndShadowPP[2] = { compile vs_2_a VShader_HemiAndSunAndShadowPP(1), compile vs_2_a VShader_HemiAndSunAndShadowPP(2) };
...
technique t0_HemiAndSunPP
{
	pass p0
	{
		...
		PixelShader = compile ps_2_a PShader_HemiAndSunPP();
	}
...
technique t0_HemiAndSunAndColorPP
{
	pass p0
	{
		...
		VertexShader = (vsArray_HemiAndSunPP[1]);
		PixelShader = compile ps_2_a PShader_HemiAndSunAndColorPP();
	}
	...
	pass p1
	{
		...
		VertexShader = (vsArray_HemiAndSunAndShadowPP[1]);
		PixelShader = compile ps_2_a PShader_HemiAndSunAndShadowAndColorPP();
	}
}
...
VS2PS_PP VShader_HemiAndSunPPtangent(APP2VStangent indata, uniform int NumBones)
{
	...
	skinSoldierForPPtangent(NumBones, indata, -sunLightDir.xyz, Pos, Normal, SkinnedLVec, wPos, outdata.HalfVec);
...
VertexShader vsArray_HemiAndSunPPtangent[2] = { compile vs_2_a VShader_HemiAndSunPPtangent(1), compile vs_2_a VShader_HemiAndSunPPtangent(2) };
VertexShader vsArray_HemiAndSunAndShadowPPtangent[2] = { compile vs_2_a VShader_HemiAndSunAndShadowPPtangent(1), compile vs_2_a VShader_HemiAndSunAndShadowPPtangent(2) };
...
VS2PS_PV VShader_HemiAndSunPV(APP2VS indata, uniform int NumBones)
{
	...
	outdata.Pos = mul(float4(Pos.xyz, 1.0), mWorldViewProj); 
 	...
 	// Hemi lookup values
	float4 wPos = mul(float4(Pos.xyz, 1.0), mWorld); 
 	outdata.GroundUV.xy = ((wPos.xyz +(hemiMapInfo.z/2) + Normal*1/*normalOffsetScale*/).xz - hemiMapInfo.xy) / hemiMapInfo.z;
	outdata.GroundUV.y = 1-outdata.GroundUV.y;
	outdata.Lerp = (Normal.y+1/*normalOffsetScale*/)/2;
	outdata.Lerp -= hemiMapInfo.w;
	
	float diff = dot(Normal, -sunLightDir.xyz);
	float3 objEyeVec = normalize(objectEyePos.xyz-Pos);
	float3 halfVec = (-sunLightDir.xyz + objEyeVec) * 0.5;
...
{
	float4 groundcolor = tex2D(sampler0, indata.GroundUV.xy);
	float4 hemicolor = lerp(groundcolor, skyColor, indata.Lerp);

	float4 totalcolor = float4(indata.DiffAndSpec.rgb*groundcolor.a*groundcolor.a+ambientColor.rgb*hemicolor.rgb, indata.DiffAndSpec.a);
	return saturate(totalcolor);
}

// Max 2 bones skinning supported!
VertexShader vsArray_HemiAndSunPV[2] = { compile vs_2_a VShader_HemiAndSunPV(1),  compile vs_2_a VShader_HemiAndSunPV(2) };
...
float4 PShader_HemiAndSunAndColorPV(VS2PS_PVCOLOR indata) : COLOR
{
	...
	float4 totalcolor = saturate(float4(indata.DiffAndSpec.rgb*groundcolor.a*groundcolor.a+ambientColor.rgb*hemicolor.rgb, indata.DiffAndSpec.a));	// Do something with spec-alpha later on
...
VS2PS_PVCOLOR_SHADOW VShader_HemiAndSunAndShadowAndColorPV(APP2VS indata, uniform int NumBones)
{
	...
	outdata.ShadowTex.z = (TexShadow2.x*outdata.ShadowTex.w)/TexShadow2.y; 	// (zL*wT)/wL == zL/wL post homo
 	...
 	// Hemi lookup values
	float4 wPos = mul(float4(Pos.xyz, 1.0), mWorld); 
 	outdata.GroundUVAndLerp.xy = ((wPos.xyz +(hemiMapInfo.z/2) + Normal*1).xz - hemiMapInfo.xy) / hemiMapInfo.z;
	outdata.GroundUVAndLerp.y = 1-outdata.GroundUVAndLerp.y;
...
float4 PShader_HemiAndSunAndShadowAndColorPV(VS2PS_PVCOLOR_SHADOW indata) : COLOR
{
	...
	float4 totalcolor = saturate(float4(indata.DiffAndSpec.rgb*totShadow+ambientColor.rgb*hemicolor.rgb, indata.DiffAndSpec.a));	// Do something with spec-alpha later on
	float4 color = tex2D(sampler1, indata.Tex0);
	totalcolor.rgb *= color.rgb;
...
// Max 2 bones skinning supported!
VertexShader vsArray_HemiAndSunAndColorPV[2] = { compile vs_2_a VShader_HemiAndSunAndColorPV(1),  compile vs_2_a VShader_HemiAndSunAndColorPV(2) };
VertexShader vsArray_HemiAndSunAndShadowAndColorPV[2] = { compile vs_2_a VShader_HemiAndSunAndShadowAndColorPV(1),  compile vs_2_a VShader_HemiAndSunAndShadowAndColorPV(2) };
...
VS2PS_PointLight_PV VShader_PointLightPV(APP2VS indata, uniform int NumBones)
{
	...
	//float4 wPos = mul(float4(Pos.xyz, 1.0), mWorld); 	
	float3 lvec = lightPos.xyz - Pos.xyz;
...
// Max 2 bones skinning supported!
//
VertexShader vsArray_PointLightPV[2] = { compile vs_2_a VShader_PointLightPV(1), 
                            compile vs_2_a VShader_PointLightPV(2) };
...
technique t0_PointLightPV
{
	pass p0
	{
		...
		PixelShader = compile ps_2_a PShader_PointLightPV();
	}
}
...
VS2PS_PointLight_PP VShader_PointLightPP(APP2VS indata, uniform int NumBones)
{
	...
	skinSoldierForPointPP(NumBones, indata, lightPos.xyz, Pos, Normal, SkinnedLVec);
...
//	float radialAtt = 1-saturate(dot(indata.SkinnedLVec,indata.SkinnedLVec)*attenuationSqrInv);
	...
	float2 intensityuv = float2(dot(indata.SkinnedLVec.xyz,expandedNormal.xyz), dot(indata.HalfVec,expandedNormal.xyz));
...
// Max 2 bones skinning supported!
//
VertexShader vsArray_PointLightPP[2] = { compile vs_2_a VShader_PointLightPP(1), 
                            compile vs_2_a VShader_PointLightPP(2) };
...
technique t0_PointLightPP
{
	pass p0
	{
		...
		PixelShader = compile ps_2_a PShader_PointLightPP();
	}
}
...
VS2PS_PointLight_PP VShader_PointLightPPtangent(APP2VStangent indata, uniform int NumBones)
{
	...
	skinSoldierForPointPPtangent(NumBones, indata, lightPos.xyz, Pos, Normal, SkinnedLVec,outdata.HalfVec);
...
// Max 2 bones skinning supported!
//
VertexShader vsArray_PointLightPPtangent[2] = { compile vs_2_a VShader_PointLightPPtangent(1), 
                            compile vs_2_a VShader_PointLightPPtangent(2) };
...
VS2PS_SpotLight_PV VShader_SpotLightPV(APP2VS indata, uniform int NumBones)
{
	...
	float radialAtt = 1-saturate(dot(lvec,lvec)*attenuationSqrInv);
	float offCenter = dot(lvecnorm, lightDir.xyz);
...
// Max 2 bones skinning supported!
//
VertexShader vsArray_SpotLightPV[2] = { compile vs_2_a VShader_SpotLightPV(1), 
                            compile vs_2_a VShader_SpotLightPV(2)
                           };
...
technique t0_SpotLightPV
{
	pass p0
	{
		...
		PixelShader = compile ps_2_a PShader_SpotLightPV();
	}
}
...
VS2PS_SpotLight_PP VShader_SpotLightPP(APP2VS indata, uniform int NumBones)
{
	...
	skinSoldierForSpotPP(NumBones, indata, lightPos.xyz, lightDir.xyz, Pos, Normal, SkinnedLVec, SkinnedLDir);
...
{	
	...
	float4 expandedNormal = tex2D(sampler1, indata.Tex0);
	expandedNormal.xyz = (expandedNormal.xyz * 2) - 1;
	float2 intensityuv = float2(dot(indata.SkinnedLVec,expandedNormal), dot(indata.HalfVec,expandedNormal.xyz));
	float4 realintensity = float4(intensityuv.rrr,pow(intensityuv.g,36)*expandedNormal.a);
...
// Max 2 bones skinning supported!
//
VertexShader vsArray_SpotLightPP[2] = { compile vs_2_a VShader_SpotLightPP(1), 
                            compile vs_2_a VShader_SpotLightPP(2)
                           };
...
technique t0_SpotLightPP
{
	pass p0
	{
		...
		PixelShader = compile ps_2_a PShader_SpotLightPP();
	}
}
...
VS2PS_SpotLight_PP VShader_SpotLightPPtangent(APP2VStangent indata, uniform int NumBones)
{
	...
	skinSoldierForSpotPPtangent(NumBones, indata, lightPos.xyz, lightDir.xyz, Pos, Normal, SkinnedLVec, SkinnedLDir,outdata.HalfVec);
...
// Max 2 bones skinning supported!
//
VertexShader vsArray_SpotLightPPtangent[2] = { compile vs_2_a VShader_SpotLightPPtangent(1), 
                            compile vs_2_a VShader_SpotLightPPtangent(2)
                           };
...
// Max 2 bones skinning supported!
//
VertexShader vsArray_MulDiffuse[2] = { compile vs_2_a VShader_MulDiffuse(1), 
                            compile vs_2_a VShader_MulDiffuse(2)
                           };
...
technique humanskinNV
{
	...
	pass preshadowed
	{
		...
		PixelShader = compile ps_2_a psSkinpreshadowedNV();
	}
	pass apply
	{
		CullMode = CCW;
