...
uniform float4x4 _ViewProj : VIEWPROJ;
uniform float4x4 _WorldViewProj : WORLDVIEWPROJ;

uniform float4x4 _ObjWorld : OBJWORLD;
uniform float4x4 _CamView : CAMVIEW;
uniform float4x4 _CamProj : CAMPROJ;

uniform float4x4 _LightViewProj : LIGHTVIEWPROJ;
uniform float4x4 _LightOccluderViewProj : LIGHTOCCLUDERVIEWPROJ;
uniform float4x4 _CamViewI : CAMVIEWI;
uniform float4x4 _CamProjI : CAMPROJI;
uniform float4 _ViewportMap : VIEWPORTMAP;
uniform float4 _ViewportMap2 : VIEWPORTMAP2;

uniform float4 _EyePos : EYEPOS;
uniform float4 _EyeDof : EYEDOF;

uniform float4 _LightWorldPos : LIGHTWORLDPOS;
uniform float4 _LightDir : LIGHTDIR;
uniform float4 _LightPos : LIGHTPOS;
uniform float4 _LightCol : LIGHTCOL;
uniform float _LightAttenuationRange : LIGHTATTENUATIONRANGE;
uniform float _LightAttenuationRangeInv : LIGHTATTENUATIONRANGEINV;

uniform float4 _ProjectorMask : PROJECTORMASK;
uniform float4 _ParaboloidZValues : PARABOLOIDZVALUES;

uniform dword _DwStencilFunc : STENCILFUNC = 3;
uniform dword _DwStencilRef : STENCILREF = 0;
uniform dword _DwStencilPass : STENCILPASS = 1;

uniform float _ShadowIntensityBias : SHADOWINTENSITYBIAS;
uniform float _LightmapIntensityBias : LIGHTMAPINTENSITYBIAS;
...
CREATE_SAMPLER(LightManager_Sampler_0, Texture_0)
CREATE_SAMPLER(LightManager_Sampler_1, Texture_1)
CREATE_SAMPLER(LightManager_Sampler_2, Texture_2)
CREATE_SAMPLER(LightManager_Sampler_3, Texture_3)
CREATE_SAMPLER(LightManager_Sampler_4, Texture_4)
CREATE_SAMPLER(LightManager_Sampler_5, Texture_5)
CREATE_SAMPLER(LightManager_Sampler_6, Texture_6)
...
float Static_Samples_Average(sampler2D Source, float2 TexCoord, float Texel)
{
	...
	StaticSamples.x = tex2D(LightManager_Sampler_4, TexCoord + float2(-Texel, -Texel * 2.0)).r;
	StaticSamples.y = tex2D(LightManager_Sampler_4, TexCoord + float2( Texel, -Texel * 2.0)).r;
	StaticSamples.z = tex2D(LightManager_Sampler_4, TexCoord + float2(-Texel,  Texel * 2.0)).r;
	StaticSamples.w = tex2D(LightManager_Sampler_4, TexCoord + float2( Texel,  Texel * 2.0)).r;
...
Sunlight_Objects_Data Sunlight_Objects_Common(in float2 TexCoord)
{
	...
	Output.ViewNormal = tex2D(LightManager_Sampler_0, TexCoord);
	Output.LightMap = tex2D(LightManager_Sampler_2, TexCoord);
	float3 ViewPosition = tex2D(LightManager_Sampler_1, TexCoord).xyz;
...
PS2FB_DiffSpec Sunlight_Transparent_PS(VS2PS_Quad Input)
{
	...
	float4 ViewNormal = tex2D(LightManager_Sampler_0, Input.TexCoord0);
	float4 WorldPos = tex2D(LightManager_Sampler_1, Input.TexCoord0);
	float4 DiffuseTex = tex2D(LightManager_Sampler_5, Input.TexCoord0);
...
Sunlight_Shadow_Dynamic_Objects_NV_Data Sunlight_Shadow_Dynamic_Objects_Common_NV(VS2PS_D3DXMesh Input)
{
	...
	Output.ViewNormal = tex2Dproj(LightManager_Sampler_0, Input.TexCoord0);
	Output.ViewPos = tex2Dproj(LightManager_Sampler_1, Input.TexCoord0);
	...
	Output.StaticSamples = Static_Samples_Average(LightManager_Sampler_4, Output.LightUV.xy, Texel);
...
PS2FB_DiffSpec Sunlight_Shadow_Dynamic_Objects_NV_PS(VS2PS_D3DXMesh Input)
{
	...
	float AvgShadowValue = tex2Dproj(LightManager_Sampler_3, Data.LightUV); // HW percentage closer filtering.
...
PS2FB_DiffSpec Sunlight_Shadow_Dynamic_1p_Objects_NV_PS(VS2PS_D3DXMesh Input)
{
	...
	float AvgShadowValue = tex2D(LightManager_Sampler_3, Data.LightUV.xy); // HW percentage closer filtering.
...
PS2FB_DiffSpec Sunlight_Shadow_Static_Objects_NV_PS(VS2PS_D3DXMesh Input)
{
	...
	float4 ViewNormal = tex2Dproj(LightManager_Sampler_0, Input.TexCoord0);
	float4 ViewPos = tex2Dproj(LightManager_Sampler_1, Input.TexCoord0);
	float4 LightMap = tex2Dproj(LightManager_Sampler_2, Input.TexCoord0);
	...
	float AvgShadowValue = tex2Dproj(LightManager_Sampler_3, LightUV); // HW percentage closer filtering.
...
Sunlight_Shadow_Dynamic_Objects_Data Sunlight_Shadow_Dynamic_Objects_Common(float4 TexCoord, float Texel, const float Epsilon)
{
	...
	Output.ViewNormal = tex2Dproj(LightManager_Sampler_0, TexCoord);
	Output.ViewPos = tex2Dproj(LightManager_Sampler_1, TexCoord);
	Output.LightUV = mul(Output.ViewPos, _LightViewProj);
	Output.LightUV.xy = clamp(Output.LightUV.xy, _ViewportMap.xy, _ViewportMap.zw);

	Output.Samples.x = tex2D(LightManager_Sampler_3, Output.LightUV.xy);
	Output.Samples.y = tex2D(LightManager_Sampler_3, Output.LightUV.xy + float2(Texel, 0.0));
	Output.Samples.z = tex2D(LightManager_Sampler_3, Output.LightUV.xy + float2(0.0, Texel));
	Output.Samples.w = tex2D(LightManager_Sampler_3, Output.LightUV.xy + float2(Texel, Texel));
...
PS2FB_DiffSpec Sunlight_Shadow_Dynamic_Objects_PS(VS2PS_D3DXMesh Input)
{
	...
	float StaticSamples = Static_Samples_Average(LightManager_Sampler_4, Data.LightUV.xy, Texel);
...
PS2FB_DiffSpec Sunlight_Shadow_Dynamic_1p_Objects_PS(VS2PS_D3DXMesh Input)
{
	...
	float TotalShadow = Static_Samples_Average(LightManager_Sampler_4, LightUV.xy, Texel);
...
Pointlight_Data Pointlight_Common(float4 TexCoord, bool Diff_Spec, bool Specular)
{
	...
	Output.CurrDiff = (Diff_Spec) ? tex2Dproj(LightManager_Sampler_0, TexCoord) : 0.0;
	Output.CurrSpec = (Diff_Spec) ? tex2Dproj(LightManager_Sampler_1, TexCoord) : 0.0;

	Output.ViewNormal = tex2Dproj(LightManager_Sampler_2, TexCoord);
	Output.ViewPos = tex2Dproj(LightManager_Sampler_3, TexCoord);
	...
	Output.RadialAtt = tex1D(LightManager_Sampler_4, Output.LightDist);
...
PS2FB_DiffSpec Pointlight_PS(VS2PS_D3DXMesh Input)
{
	...
	// float4 LightMap = tex2Dproj(LightManager_Sampler_5, Input.TexCoord0);
...
PS2FB_DiffSpec Pointlight_Shadow_PS(VS2PS_D3DXMesh Input)
{
	...
	float2 ParaSamples;
	ParaSamples.x = tex2D(LightManager_Sampler_5, PosData.ParaPos1.xy);
	ParaSamples.y = tex2D(LightManager_Sampler_6, PosData.ParaPos2.xy);
...
PS2FB_DiffSpec Pointlight_Shadow_NV_PS(VS2PS_D3DXMesh Input)
{
	...
	float2 AvgPara;
	AvgPara.x = tex2Dproj(LightManager_Sampler_5, float4(PosData.ParaPos1.xy, PosData.ParaPosZ, 1.0));
	AvgPara.y = tex2Dproj(LightManager_Sampler_6, float4(PosData.ParaPos2.xy, PosData.ParaPosZ, 1.0));
...
Spotlight_Data Spotlight_Common(float4 TexCoord)
{
	...
	Output.ConicalAtt = tex1D(LightManager_Sampler_5, 1.0 - (Output.FallOff * Output.FallOff));
...
PS2FB_DiffSpec Spotlight_PS(VS2PS_D3DXMesh Input)
{
	...
	float4 CurrDiff = tex2Dproj(LightManager_Sampler_0, Input.TexCoord0);
	float4 CurrSpec = tex2Dproj(LightManager_Sampler_1, Input.TexCoord0);
...
PS2FB_DiffSpec Spotlight_Shadow_PS(VS2PS_D3DXMesh Input)
{
	...
	float4 CurrDiff = tex2Dproj(LightManager_Sampler_0, Input.TexCoord0);
	float4 CurrSpec = tex2Dproj(LightManager_Sampler_1, Input.TexCoord0);
	float4 Samples = tex2D(LightManager_Sampler_6, LightUV.xy);
...
PS2FB_DiffSpec Spot_Projector_PS(VS2PS_D3DXMesh Input)
{
	...
	float4 Headlight = tex2D(LightManager_Sampler_5, LightUV.xy);
...
PS2FB_DiffSpec Spot_Projector_Shadow_Common_PS(VS2PS_D3DXMesh Input, bool IsNV40)
{
	...
	float4 Headlight = tex2D(LightManager_Sampler_5, HeadlightUV);
	...
	float4 Samples;
	Samples.x = tex2D(LightManager_Sampler_6, LightUV.xy);
	Samples.y = tex2D(LightManager_Sampler_6, LightUV.xy + float2(Texel, 0.0));
	Samples.z = tex2D(LightManager_Sampler_6, LightUV.xy + float2(0.0, Texel));
	Samples.w = tex2D(LightManager_Sampler_6, LightUV.xy + float2(Texel, Texel));
...
PS2FB_DiffSpec Spot_Projector_Shadow_NV_PS(VS2PS_D3DXMesh Input)
{
	...
	float4 AvgSamples = tex2D(LightManager_Sampler_6, LightUV.xy);
...
{
	PS2FB_DiffSpec Output;
	Output.Col0 = tex2Dproj(LightManager_Sampler_0, Input.TexCoord0);
	Output.Col1 = tex2Dproj(LightManager_Sampler_1, Input.TexCoord0);
	return Output;
}
...
PS2FB_Combine Combine_Common_PS(VS2PS_Quad Input, bool IsTransparent)
{
	...
	float4 LightMap = tex2D(LightManager_Sampler_0, Input.TexCoord0);
	float4 DiffuseTex = tex2D(LightManager_Sampler_1, Input.TexCoord0);
	float4 Diff1 = tex2D(LightManager_Sampler_2, Input.TexCoord0);
	float4 Spec1 = tex2D(LightManager_Sampler_3, Input.TexCoord0);
